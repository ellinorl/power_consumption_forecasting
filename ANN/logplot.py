...
log_path_csv = 'logs/ANN_log_2021511_9_52_49.csv'
...
if plot_3d_x:  # If plot_3d_x contains value
    for a in log_df['activation'].unique():  # Make one heatmap for each unique activation function
        ...
        for i in range(len(plot_3d_x)):
            ...
            uniqueX = np.flip(xData.unique())  # Get unique values from x-data in reversed order
            uniqueY = yData.unique()  # Get unique values from y-data
            ...
            fig, ax = plt.subplots(figsize=(15, 8))  # Create figure window with custom size
            fig.canvas.set_window_title(a+': '+xlabel+' vs '+ylabel)  # Set window title
            ax.set_title('Färgkarta över MSE för ANN')  # Set plot title
            fig.tight_layout = True  # Use tight layout in figure
            im = ax.imshow(map, cmap=results.Results.get_continuous_cmap(results.Results(), ['#FFFFFF', '#ED553B', '#000000'], [0, 0.15, 1]))  # Heatmap plot using custom color scale
            ax.set_xlabel('Seriestorlek')  # Set x-label
            ax.set_ylabel('Inlärningshastighet')  # Set y-label
            cbar = ax.figure.colorbar(im, ax=ax)  # Create color bar for heatmap
            cbar.ax.set_ylabel('MSE [(kW)$^2$]')  # Set color bar y-label
